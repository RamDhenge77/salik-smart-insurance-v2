...
interface SellCarProgressProps {
  currentStep: number;
}

const SellCarProgress: React.FC<SellCarProgressProps> = ({ currentStep }) => {
  ...
  const steps = [
    { title: "Car Details", icon: <CarFront className="h-5 w-5" /> },
    { title: "Upload Photos", icon: <CarFront className="h-5 w-5" /> },
    { title: "Selling Method", icon: <CarFront className="h-5 w-5" /> },
    { title: "Confirmation", icon: <CarFront className="h-5 w-5" /> },
  ];

  const calculatePosition = (stepIndex: number) => {
    // For step 1, position is 0%
    // For step 4, position is 100%
    const basePosition = (stepIndex / (steps.length - 1)) * 100;

    // For current step, we animate from previous to current position
    if (stepIndex + 1 === currentStep) {
      const previousPosition = ((stepIndex - 1) / (steps.length - 1)) * 100;
      return animationComplete ? `${basePosition}%` : `${previousPosition}%`;
    }
    ...
    <div className="mb-12">
      <div className="text-center mb-2">
        <h2 className="text-2xl font-bold text-[#2596be]">
          Selling Your Lexus GX460
        </h2>
        <p className="text-gray-600">
          Complete the journey to list your luxury SUV
        </p>
      </div>

      {/* Car Journey Animation */}
      <div className="relative mt-10 mb-8 mx-9">
        <div className="h-3 bg-gray-200 rounded-full w-full relative">
          <div
            className="h-3 bg-[#2596be] rounded-full absolute top-0 left-0 transition-all duration-500 ease-out"
            style={{
          ...
          <div className="absolute top-1/2 left-0 w-full h-px">
            <div className="w-full h-px flex justify-between items-center px-2">
              {[...Array(10)].map((_, i) => (
                <div key={i} className="w-6 h-px bg-white"></div>
              ))}
            </div>
          </div>
        </div>
        ...
        >
          <div className="animate-bounce">
            <CarFront className={`h-6 w-6 text-[#2596be]`} />
          </div>
          <div className="h-4 w-0.5 bg-[#2596be] mx-auto" />
        </div>
        ...
        {steps.map((step, index) => (
          <div
            key={index}
            className="absolute -bottom-12 transform -translate-x-1/2 transition-opacity duration-300"
            style={{
          ...
          >
            <div
              className={`w-5 h-5 rounded-full mx-auto mb-1 ${
                index + 1 <= currentStep ? "bg-[#2596be]" : "bg-gray-300"
      ...
      {/* Car details preview - only visible from step 2 onwards */}
      {currentStep > 1 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 mt-8 max-w-lg mx-auto">
          <div className="flex items-center space-x-4">
            <div className="bg-salik-light/30 rounded-lg p-2">
              <CarFront className="h-10 w-10 text-[#2596be]" />
            </div>
            <div>
              <h3 className="font-semibold">Lexus GX460</h3>
              <p className="text-sm text-gray-600">
                2022 · Luxury SUV · 18,500 km
              </p>
            </div>
            <div className="ml-auto">
              <p className="font-semibold text-[#2596be] flex items-center gap-1"><svg
              xmlns="http://www.w3.org/2000/svg"
              id="UaeDirham"
...
export default SellCarProgress;
