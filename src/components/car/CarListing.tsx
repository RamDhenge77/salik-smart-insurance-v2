...
import { Calendar, Car as CarIcon, MapPin, Gauge } from "lucide-react";

interface CarListingProps {
  car: Car;
  onPurchaseClick: (car: Car) => void;
}

const CarListing: React.FC<CarListingProps> = ({ car, onPurchaseClick }) => {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <div className="aspect-video relative overflow-hidden">
        ...
        />
        <Badge className="absolute top-2 right-2 bg-[#2596be] hover:bg-[#2596be]">
          {car.seller}
        </Badge>
      </div>
      ...
      <CardContent className="pb-2">
        <div className="grid grid-cols-2 gap-y-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm">{formatMileage(car.mileage)} mi</span>
          </div>
          <div className="flex items-center gap-1.5">
      ...
      <CardFooter className="pt-2">
        <Button
          className="w-full bg-btn-primary hover:bg-btn-secondary text-btn-textPrimary"
